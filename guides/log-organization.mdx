---
title: "Organizing Chromologger Log Files by Date and Purpose"
sidebarTitle: "Log Organization"
description: "Structure your log files by date, component, or environment using the log_file_name constructor argument to keep logs scannable and easy to archive."
---
...
<Tip>
  Chromologger resolves the file path with `pathlib.Path.resolve().absolute()`
  internally, so both relative paths (e.g. `./logs/app.log`) and absolute paths
  are handled correctly on all platforms — including Windows, where path
  separators differ.
</Tip>
